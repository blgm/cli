...
func (cmd ShareRouteCommand) Execute(args []string) error {
	...
	if err != nil {
		if _, ok := err.(actionerror.RouteNotFoundError); ok {
			cmd.UI.DisplayText("Route to be shared does not exist.")
			return err
		}
	}

	destinationOrg := cmd.DestinationOrg

	if destinationOrg == "" {
		destinationOrg = cmd.Config.TargetedOrganizationName()
	}

	targetedSpace, warnings, err := cmd.Actor.GetSpaceByNameAndOrganization(cmd.DestinationSpace, destinationOrg)
	if err != nil {
		if _, ok := err.(actionerror.SpaceNotFoundError); !ok {
			cmd.UI.DisplayText("Destination Space does not exist")
			return err
		}
	}

	url := desiredURL(domain.Name, cmd.Hostname, path, 0)
	cmd.UI.DisplayTextWithFlavor("Sharing rouute {{.URL}} to space {{.DestinationSpace}} as {{.User}}",
