...
var _ = Describe("share route command", func() {
	Context("Help", func() {
		It("appears in cf help -a", func() {
			...
			Expect(session).To(HaveCommandInCategoryWithDescription("share-route", "ROUTE", "Share a route in between spaces"))
		})
				...
				When("the route exists", func() {
					...
					When("the target space exists in targeted org", func() {
						BeforeEach(func() {
							...
							Eventually(helpers.CF("create-space", targetSpaceName)).Should(Exit(0))
						})
					...
					When("the target space exists in another existing org", func() {
						...
						BeforeEach(func() {
							...
							domain.Create()
							Eventually(helpers.CF("create-org", targetOrgName)).Should(Exit(0))
							Eventually(helpers.CF("create-space", targetSpaceName, "-o", targetSpaceName)).Should(Exit(0))
						})
